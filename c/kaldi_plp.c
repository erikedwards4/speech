...
int kaldi_plp_s (float *Y, float *X, const size_t N, const float sr, const float frame_length, const float frame_shift, const int snip_edges, const float dither, const int dc0, const int raw_energy, const float preemph, const char win_type[], const float lof, const float hif, const size_t B, const float compress, const size_t C, const float Q, const float cep_scale, const int use_energy, const int mn0)
{
    ...
    const size_t P = B - 1u;                        //polynomial order
    const float athresh = (float)sqrt(1.0-1e-5);    //Kaldi precaution
    float *AR, *A2, a, e;
    if (!(AR=(float *)malloc(P*sizeof(float)))) { fprintf(stderr,"error in kaldi_plp_s: problem with malloc. "); perror("malloc"); return 1; }
    ...
    *--lift *= 0.5f*cep_scale/sqrtf((float)B);    //proper DC scaling and cepstral_scale
    ...
    for (size_t w=W; w>0u; --w)
    {
        ...
        *AR = *X / e; a = -*AR;
        ...
        {
            a = *Yi;
            for (size_t q=p; q>0u; --q, ++AR) { Yi-=2; a -= *Yi * *AR; }
            a /= -e; *AR = -a;
            for (size_t q=p; q>0u; --q, ++A2) { --AR; *A2 = *AR; }
            AR += p;
            for (size_t q=p; q>0u; --q) { --A2; --AR; *AR += a * *A2; }
            if (a<athresh) { e *= 1.0f - a*a; }
            else { e *= 0.00001f; }  //Kaldi precaution
        }
        Yi -= 2u*Ly;
        ...
        *Yd++ = logf(e+preg);
        *Yd++ = *AR;
        for (size_t k=2u; k<Ly; ++k, ++Yd)
        {
            sm = 0.0f;
            for (size_t j=1u; j<k; ++j, ++AR) { sm += (float)(k-j) * *AR * *(Yd-j-1u); }
            *Yd = *AR - sm/(float)k;
            AR -= k - 1u;
        }
    ...
    free(eql); free(AR); free(A2); free(lift);
