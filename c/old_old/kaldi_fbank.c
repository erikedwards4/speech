...
int kaldi_fbank_default (float *Y, float *X, const size_t N, const float frame_shift, const int snip_edges, const float dither, const int dc0, const int raw_energy, const float preemph, const char win_type[], const int amp, const float lof, const float hif, const size_t B, const int lg, const int use_energy, const int mn0);
    ...
    for (size_t w=W; w>0u; --w)
    {
        ...
        if (dither>FLT_EPS)
        {
            for (size_t l=0u; l<L; l+=2u)
            {
                ...
                Xw[2u*l] += R * cos(M_2PI*u2);
                Xw[2u*l+1u] += R * sin(M_2PI*u2);
            }
        }
        ...
        Yf -= 2u;
        for (size_t f=0u; f<F-2u; ++f, --Yf) { Yf[F-2u-f] += Yw[f] * Yw[f]; }
...
int kaldi_fbank_default (float *Y, float *X, const size_t N, const float sr, const float frame_length, const float frame_shift, const int snip_edges, const float dither, const int dc0, const int raw_energy, const float preemph, const char win_type[], const int amp, const float lof, const float hif, const size_t B, const int lg, const int use_energy, const int mn0)
{
    if (N<1u) { fprintf(stderr,"error in kaldi_fbank_default: N (nsamps in signal) must be positive\n"); return 1; }
    if (sr<FLT_EPSILON) { fprintf(stderr,"error in kaldi_fbank_default: sr must be positive\n"); return 1; }
    if (frame_length<FLT_EPSILON) { fprintf(stderr,"error in kaldi_fbank_default: frame_length must be positive\n"); return 1; }
    if (frame_shift<FLT_EPSILON) { fprintf(stderr,"error in kaldi_fbank_default: frame_shift must be positive\n"); return 1; }
    if (dither<0.0f) { fprintf(stderr,"error in kaldi_fbank_default: dither must be nonnegative\n"); return 1; }
    if (preemph<0.0f) { fprintf(stderr,"error in kaldi_fbank_default: preemph coeff must be nonnegative\n"); return 1; }
    if (lof<0.0f) { fprintf(stderr,"error in kaldi_fbank_default: lof (low-freq cutoff) must be nonnegative\n"); return 1; }
    if (hif<=lof) { fprintf(stderr,"error in kaldi_fbank_default: hif (high-freq cutoff) must be > lof\n"); return 1; }
    if (B<1u) { fprintf(stderr,"error in kaldi_fbank_default: B (num mel bins) must be positive\n"); return 1; }
    if (snip_edges && N<400u) { fprintf(stderr,"error in kaldi_fbank_default: frame length must be < signal length if snip_edges\n"); return 1; }
    ...
    //Set stp (frame_shift in samps)
    const size_t stp = (size_t)(sr*frame_shift/1000.0f);
    if (stp<1u) { fprintf(stderr,"error in kaldi_fbank_default: frame_shift must be greater than 1 sample\n"); return 1; }
    ...
    {
        //Init random num generator
        if (timespec_get(&ts,TIME_UTC)==0) { fprintf(stderr, "error in kaldi_fbank_default: problem with timespec_get.\n"); perror("timespec_get"); return 1; }
        state = (uint64_t)(ts.tv_nsec^ts.tv_sec) + inc;
    }
    ...
    else
    {
        ...
        {
            fprintf(stderr,"error in kaldi_fbank_default: window type must be rectangular, blackman, hamming, hann or povey (lower-cased)\n"); return 1;
        }
    }
    ...
    float Xw[512], Yw[512];
    fftwf_plan plan = fftwf_plan_r2r_1d((int)nfft,Xw,Yw,FFTW_R2HC,FFTW_ESTIMATE);
    if (!plan) { fprintf(stderr,"error in kaldi_fbank_default: problem creating fftw plan"); return 1; }
    for (size_t nf=0u; nf<nfft; ++nf) { Xw[nf] = 0.0f; }
    ...
    beg_f = (size_t *)malloc(B*sizeof(size_t));
    num_f = (size_t *)malloc(B*sizeof(size_t));
    if (!mels) { fprintf(stderr,"error in kaldi_fbank_default: problem with malloc. "); perror("malloc"); return 1; }
    if (!F2B) { fprintf(stderr,"error in kaldi_fbank_default: problem with malloc. "); perror("malloc"); return 1; }
    if (!beg_f) { fprintf(stderr,"error in kaldi_fbank_default: problem with malloc. "); perror("malloc"); return 1; }
    if (!num_f) { fprintf(stderr,"error in kaldi_fbank_default: problem with malloc. "); perror("malloc"); return 1; }
    for (size_t f=0; f<F; ++f) { mels[f] = 1127.0f * logf(1.0f+(float)f*finc/700.0f); }
    lmel = lomel; cmel = lmel + dmel; rmel = cmel + dmel;
    for (size_t b=B; b>0u; --b)
    {
    ...
    {
        float *mns;
        if (!(mns=(float *)calloc(B,sizeof(float)))) { fprintf(stderr,"error in kaldi_fbank_default: problem with calloc. "); perror("calloc"); return 1; }
        for (size_t w=W; w>0u; --w, mns-=B)
        {
            for (size_t b=B; b>0u; --b, ++mns) { *mns += *--Y; }
        }
    ...
    //Free
    free(mels); free(F2B); free(beg_f); free(num_f);
