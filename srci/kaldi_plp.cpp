...
//Description
string descr;
descr += "Makes Kaldi PLP features for univariate, real-valued X.\n";
descr += "These are cepstral coeffs via perceptual linear prediction (PLP).\n";
descr += "\n";
...
descr += "is transformed to power in B mel frequency bands. Then LP recursions\n";
descr += "obtain the cepstral coeffs (CCs) and final PLP output.\n";
descr += "\n";
descr += "The output Y has size CxW or WxC, where C is the number of CCs,\n";
...
descr += "\n";
descr += "Examples:\n";
descr += "$ kaldi_plp -b40 X -o Y \n";
descr += "$ kaldi_plp -e X > Y \n";
descr += "$ cat X | kaldi_plp -e -z - > Y \n";
descr += "$ kaldi_plp -r8000 -d0 -w'hamming' -e -z X -o Y \n";
...
if (o1.T==1u)
{
    ...
    if (codee::kaldi_plp_s(Y,X,i1.N(),float(sr),float(fl),float(shft),snipe,float(d),dc0,rawe,float(p),wintype.c_str(),float(lof),float(hif),B,C,Q,usee,mn0))
